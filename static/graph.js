...
class Graph {
  ...
  __parseNeighbors() {
    ...
    for (let edge of this.edges) {
      ...
      if (!this.neighbors.has(vertex0)) {
        ...
        this.neighbors.set(
          vertex0,
          this.neighbors.get(vertex0).concat(vertex1)
        );
      }
    }
    ...
    for (let edge of this.edges) {
      ...
      if (!this.neighbors.has(vertex1)) {
        ...
        this.neighbors.set(
          vertex1,
          this.neighbors.get(vertex1).concat(vertex0)
        );
      }
    }
  ...
  A_star(start, goal, heuristic) {
    ...
    const f = (thisVertex, nextVertex) =>
      g(thisVertex, nextVertex) + heuristic(thisVertex, nextVertex);
    ...
    while (openSet.length > 0) {
      ...
      for (let neighbor of this.neighbors.get(current)) {
        ...
        let tentative_gScore =
          gScore.get(current) + current.getDistance(neighbor);
